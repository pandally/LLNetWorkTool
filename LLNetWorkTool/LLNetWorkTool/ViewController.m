...
- (void)viewDidLoad {
    ...
    NSLog(@"网络缓存大小cache = %.2fMB",[LLNetworkCache getAllHttpCacheSize]/1024/1024.f);
    
    //检查网络状态
    [LLNetworkTool checkNetworkStatusWithBlock:^(LLNetworkStatus status) {
        switch (status) {
            case LLNetworkStatusUnknown:
...
- (void)getData:(BOOL)isOn url:(NSString *)url
{
    ...
    if(isOn)
    {
        ...
        [LLNetworkTool GET:url parameters:nil responseCache:^(id responseCache) {
    ...
    else
    {
        ...
        [LLNetworkTool GET:url parameters:nil success:^(id responseObject) {
...
- (IBAction)download:(UIButton *)sender {
    ...
    if(!self.isDownload)
    {
        ...
        task = [LLNetworkTool downloadWithURL:downloadUrl fileDir:@"Download" progress:^(NSProgress *progress) {
