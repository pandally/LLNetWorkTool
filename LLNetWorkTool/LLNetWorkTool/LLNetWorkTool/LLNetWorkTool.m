    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown:
                _status ? _status(LLNetworkStatusUnknown) : nil;
                _isNetwork = NO;
                NSLog(@"未知网络");
                break;
            case AFNetworkReachabilityStatusNotReachable:
                _status ? _status(LLNetworkStatusNotReachable) : nil;
                _isNetwork = NO;
                NSLog(@"无网络");
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN:
                _status ? _status(LLNetworkStatusReachableViaWWAN) : nil;
                _isNetwork = YES;
                NSLog(@"手机自带网络");
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi:
                _status ? _status(LLNetworkStatusReachableViaWiFi) : nil;
                _isNetwork = YES;
                NSLog(@"WIFI");
                break;
        }
...
{
    //读取缓存
    responseCache ? responseCache([LLNetworkCache getHttpCacheForKey:URL]) : nil;
    ...
    return [manager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        //对数据进行异步缓存
        responseCache ? [LLNetworkCache saveHttpCache:responseObject forKey:URL] : nil;
...
+ (NSURLSessionTask *)uploadWithURL:(NSString *)URL
                         parameters:(NSDictionary *)parameters
                           ...
                           progress:(HttLLrogress)progress
                            success:(HttpRequestSuccess)success
                            failure:(HttpRequestFailed)failure
{
